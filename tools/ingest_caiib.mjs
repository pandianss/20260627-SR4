// Transforms the raw CAIIB authoring data (content/raw/caiib/*.js) into a
// content pack in the engine schema (same shape the Dart compiler emits),
// written to packages/app/assets/content_pack_caiib.json.
//
// Run: node tools/ingest_caiib.mjs
// Validate afterwards: dart run packages/domain/bin/validate_pack.dart <pack>

import { SUBJECTS, ELECTIVES, MODULES, TOPICS } from '../content/raw/caiib/contentGraph.js';
import { MICRO_LESSONS } from '../content/raw/caiib/microLessons.js';
import { QUESTION_BANK } from '../content/raw/caiib/questionBank.js';
import { writeFileSync, mkdirSync } from 'node:fs';
import { dirname, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

const __dirname = dirname(fileURLToPath(import.meta.url));
const OUT = resolve(__dirname, '../packages/app/assets/content_pack_caiib.json');
...
const subjects = [
  ...SUBJECTS.map((s) => ({ ...s, kind: 'compulsory' })),
  ...ELECTIVES.map((s) => ({ ...s, kind: 'elective' })),
];
...
function makeQuestion(id, topicId, subjectId, difficulty, q, opts, correct, why) {
  ...
  return {
    ...
    authoring: { status: 'published', authorId: 'caiib_ingest', reviewerId: 'caiib_review' },
    payload: {
      type: 'mcq_single',
      stem: L(stem),
...
console.log(`CAIIB content pack -> ${OUT}`);
